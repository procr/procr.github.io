...
具体实现来说，为这些内存页再分配一块内存区域，这块区域由VM和hypervisor共享，实际上这一区域存放一个大数组，数组的每个元素存放一些关于对应下标的内存页的使用信息。实际上用已有的spin_lock也是可以的，不过spin_lock 是用xadd指令实现的，在guest-hypervisor这样的组合中，貌似会有问题，具体问题出现在什么地方还不知道，本文暂不关注。
